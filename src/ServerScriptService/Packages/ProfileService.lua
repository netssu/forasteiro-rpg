				...
				if IsThisSession(active_session) == true then
					...
					elseif aggressive_steal == true then
						...
						if type(not_released_handler) == "function" then
							...
							-- This case should never happen because the string cases ("ForceLoad" and "Steal")
							-- are handled earlier in the code, but adding this for type safety
							error("[ProfileService]: Unexpected state - not_released_handler is not a function")
						end
					end
				end
			else
