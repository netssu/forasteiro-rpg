...
local function get_shop_ui()
	...
	if not playerGui then 
		warn("[SHOP] PlayerGui não encontrado!")
		return nil 
	end

	local UI = playerGui:WaitForChild("UI", 10)
	if not UI then 
		warn("[SHOP] UI não encontrado!")
		return nil 
	end

	local shopFrame = UI:FindFirstChild("Shop", true)
	if not shopFrame then 
		warn("[SHOP] Shop frame não encontrado!")
		return nil 
	end
...
local function prompt_purchase(productId: number, productType: string, productName: string)
	...
	elseif productType == "Gamepass" then
		...
		warn("[SHOP] Tipo de produto inválido:", productType)
...
local function setup_purchase_button(button: GuiButton)
	...
	if not productKey or not productType then
		warn("[SHOP] Botão sem atributos configurados:", button:GetFullName())
		return
	end
	...
	if not productData or not productId then
		warn("[SHOP] Produto não encontrado para:", productKey, productType)
		return
	end
	...
	print("[SHOP] Botão configurado:", productKey, "-", productType)
end
...
local function setup_pogos_button(shopFrame: Frame)
	...
	print("[SHOP] Botão Pogos configurado")
end

local function initialize_shop()
	local shopFrame = get_shop_ui()
	if not shopFrame then
		warn("[SHOP] Não foi possível inicializar a loja")
		return
	end
	...
	if #purchaseButtons == 0 then
		warn("[SHOP] Nenhum botão PurchaseBT encontrado!")
		return
	end
	...
	print("[SHOP] Loja inicializada com sucesso! Total de botões:", #purchaseButtons)
end
