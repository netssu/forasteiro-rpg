...
function VoxBreaker:VoxelizePart(Part : Part, MinimumVoxelSize : number, TimeToReset : number)
	...
	repeat
		...
		local check = true
		for i, v in pairs(parts) do
			if v.Size.X > minimum or v.Size.Y > minimum or v.Size.Z > minimum then
				check = false
			end
		end
	until check == true
...
function VoxBreaker:CreateHitbox(Size : Vector3 | string, Cframe : CFrame , Shape : Enum.PartType|MeshPart , MinimumVoxelSize : number , TimeToReset : number, Params : OverlapParams)


	local DefaultSize = Vector3.new(1,1,1)
	local DefaultShape = Enum.PartType.Block
	local DefaultTimeToReset = 50 
...
function VoxBreaker.CreateMoveableHitbox(MinimumVoxelSize : number | string, TimeToReset : number, Size : Vector3, Cframe : CFrame , Shape : Enum.PartType|MeshPart, Params : OverlapParams)
	local self = {}
...
function VoxBreaker:GetCachedPart() 
	-- cache é a variável local que o VoxBreaker criou no topo do script dele
	if cache then
		return cache:GetPart()
	else
		-- Fallback caso o cache esteja desativado nas configurações do módulo
