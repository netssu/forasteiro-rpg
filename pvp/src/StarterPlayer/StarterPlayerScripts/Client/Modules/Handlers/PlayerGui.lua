...
local StartingCash = UserData:FindFirstChild("Money").Value
local StartingEXP = UserData:FindFirstChild("EXP").Value
...
local function setupHotbar()
	...
	for _, Slot in ipairs(InventoryFolder:GetChildren()) do
		...
		if Slot.Value == "" then UISlot:FindFirstChild("Holder").Visible = false continue end
		
		local ImageId = TowerData[Slot.Value].ImageId
		if not ImageId then continue end
		
		local Price = TowerData[Slot.Value].Price
		if not Price then UISlot:FindFirstChild("Holder").Visible = false continue end
...
local function handleClientTowers()
	...
	GameRemotes:WaitForChild("ReEnableInfo").OnClientEvent:Connect(function(tower)
		...
		local currentPriorityIndex = table.find(Priorities, tower:GetAttribute("Priority")) or 1
		PriorityDisplayText.Text = Priorities[currentPriorityIndex]
		...
		PriorityForwardButton.MouseButton1Click:Connect(function()
			...
			local newPriority = Priorities[currentPriorityIndex]
			PriorityDisplayText.Text = newPriority
			tower:SetAttribute("Priority", newPriority)
		end)
...
local function handleSpawnMenu()
	local EnemyTable = require(ReplicatedStorage.Modules.StoredData.EnemyStats)
	if not EnemyTable then return end

	local ShowButton = MainGui:WaitForChild("Spawn"):WaitForChild("Show")
	if not ShowButton then return end

	local SpawnMenu = MainGui:WaitForChild("Spawn"):WaitForChild("Hotbar")
	if not SpawnMenu then return end

	local ScrollingFrame = SpawnMenu:WaitForChild("ScrollingFrame")
	...
	local function setupTemplates()
		...
		local enemies = {}
		for name, data in pairs(EnemyTable) do
			table.insert(enemies, {Name = name, Data = data})
		end

		table.sort(enemies, function(a, b)
			local healthA = a.Data.Health or 0
			local healthB = b.Data.Health or 0
			return healthA < healthB
		end)

		for _, enemy in ipairs(enemies) do
			local enemyName = enemy.Name
			local enemyData = enemy.Data

			local clone = Template:Clone()
			local Image = clone:FindFirstChild("UnitIcon")
			clone.Name = enemyName
			clone.Visible = true
			clone.Parent = ScrollingFrame

			if clone:FindFirstChild("Health") and enemyData.Health then
				clone.Health.Text = tostring(enemyData.Health)
			end

			local priceLabel = clone:FindFirstChild("Holder") and clone.Holder:FindFirstChild("Price")
			if priceLabel and enemyData.PriceToSpawn then
				priceLabel.Text = "$" .. Digits.AddCommas(enemyData.PriceToSpawn)
			end
			
			Image.Image = "rbxassetid://"..enemyData.ImageId

			clone.MouseButton1Click:Connect(function()
				if not canSpawn then
					sendNotification(`Please wait {(lastspawned + 2) - os.time()} seconds!`, "Error")
					return
				end

				canSpawn = false
				lastspawned = os.time()
				--sendNotification("Spawned " .. enemyName, "Success")
				Remotes.PVP.SpawnUnit:FireServer(enemyName)
	...
	ShowButton.MouseButton1Click:Connect(function()
		...
		SpawnMenu.Visible = showingUI
		ShowButton.TextLabel.Text = showingUI and "Close" or "Spawn Enemies"
	end)
